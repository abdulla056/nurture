...
custom_session_interface = CustomRedisSessionInterface(
    redis_client=redis_client,
    key_prefix="flask_session:",  # Unique prefix for session keys
...
def verify_token():
    try:
        ...
        redirect = 'login'
        session_id = custom_session_interface.create_session(email, redirect)
        if session_id:
            print(session_id)
            return jsonify({
...
def send_email():
    try:
        ...
        session_data = custom_session_interface.get_session_by_sid(session_id)
        if session_data and session_data['redirect'] == 'login':
            ...
            custom_session_interface.update_session(session_id, session_data)
    ...
    try:
        ...
        if session_data and session_data['redirect'] == 'login':
            ...
            if int(code) == int(session_data['mfa']):
                ...
                custom_session_interface.delete_session_by_sid(session_id) 
